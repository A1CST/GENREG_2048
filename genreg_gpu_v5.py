# ================================================================
# GENREG 2048 — GPU Evolver V5 (Evolved Reproduction + Crossover)
# ================================================================
# Extends V4 with sexual reproduction (crossover between genomes).
#
# V4: per-genome mutation strategy (mut_rate, mut_scale, explore)
# V5 adds: when a child is created, it can be a CROSS of two parents
#   instead of a mutated clone of one parent.
#
# Crossover strategy:
#   - Each child has a probability of being a crossover (vs pure clone)
#   - When crossing, pick TWO elite parents
#   - For each weight tensor, randomly choose parent A or parent B
#     at the neuron level (not weight level — keeps neurons coherent)
#   - Encoder from parent A, controller from parent B, or mixed
#   - Reproductive traits inherited from one parent (random)
#
# Why neuron-level crossover:
#   - Weight-level crossover destroys learned features
#   - Neuron-level preserves intact features from both parents
#   - Like biological crossover at the chromosome level, not base-pair
#
# The crossover probability is ITSELF an evolvable trait (like V4's
# mut_rate). Genomes that benefit from crossover will evolve higher
# crossover rates. Those that don't will evolve it to zero.
# ================================================================
...
class GPUEvolverV5(GPUEvolver):
    ...
    def _pack_from_population_v3(self, population):
        ...
        # --- V4: Per-genome reproductive strategy ---
        # These are evolvable traits that determine HOW each genome's
        # children are mutated. They're heritable and mutable themselves.
        # Load from genome if stored, otherwise initialize with diversity.
        self.genome_mut_rate = torch.zeros(B, device=d)
        self.genome_mut_scale = torch.zeros(B, device=d)
        self.genome_explore = torch.zeros(B, device=d)
        self.genome_crossover = torch.zeros(B, device=d)  # V5: crossover probability

        for i, g in enumerate(genomes):
            self.genome_mut_rate[i] = getattr(g, 'repro_mut_rate', 0.03 + torch.rand(1).item() * 0.07)
            self.genome_mut_scale[i] = getattr(g, 'repro_mut_scale', 0.05 + torch.rand(1).item() * 0.15)
            self.genome_explore[i] = getattr(g, 'repro_explore', torch.rand(1).item() * 0.1)
            self.genome_crossover[i] = getattr(g, 'repro_crossover', 0.1 + torch.rand(1).item() * 0.3)
    ...
    def evolve(self, trust, survival_pct=20, trust_inherit=0.1,
        ...
        # Reorder all tensors (encoder + controller + proteins + reproductive traits)
        all_attrs = [
            "enc_w", "enc_b", "act_p1", "act_p2", "act_p3", "act_p4",
            "w1", "b1", "w2", "b2",
            "trend_momentum", "trust_gain", "trust_scale", "trust_decay",
            "genome_mut_rate", "genome_mut_scale", "genome_explore",
            "genome_crossover",
        ]
        ...
        # --- V5: CROSSOVER ---
        # For each child (bottom k), decide: clone or crossover?
        # If crossover, pick a SECOND parent from elite and blend.
        # Blending is at the NEURON level: for each neuron in each layer,
        # randomly pick which parent's neuron to use. This preserves
        # learned features intact while combining them.
        n_keep = len(keep_idx)
        child_cross_prob = self.genome_crossover[n_keep:]  # (k,)
        cross_mask = torch.rand(k, device=self.dev) < child_cross_prob  # which children are crossovers

        if cross_mask.any():
            n_cross = cross_mask.sum().item()
            # Pick second parents from elite (different from first parent)
            parent2_choices = torch.multinomial(weights, k, replacement=True)
            parent2_idx = elite_idx[parent2_choices]

            # For each crossing child, blend tensors neuron-by-neuron
            for attr in ["enc_w", "enc_b", "w1", "b1", "w2", "b2",
                          "act_p1", "act_p2", "act_p3", "act_p4"]:
                child_tensor = getattr(self, attr)[n_keep:]  # (k, ...)
                parent2_tensor = getattr(self, attr)[parent2_idx]  # (k, ...) from second parents — but wait, parent2 might be in elite range

            # Need parent2 data BEFORE reorder happened. Since we already reordered,
            # parent2_idx points into the reordered population. Elite are at positions 0:k.
            # So parent2_tensor from the current state IS valid elite genomes.
            for attr in ["enc_w", "enc_b", "w1", "b1", "w2", "b2",
                          "act_p1", "act_p2", "act_p3", "act_p4"]:
                tensor = getattr(self, attr)
                child_data = tensor[n_keep:]            # (k, ...) — children (cloned from parent1)
                parent2_data = tensor[parent2_idx]      # (k, ...) — second parents from elite

                if child_data.dim() == 1:
                    # (k,) — scalar per genome, just swap some
                    swap = cross_mask
                    child_data[:] = torch.where(swap, parent2_data, child_data)
                elif child_data.dim() == 2:
                    # (k, neurons) — swap entire neurons
                    neuron_mask = torch.rand_like(child_data) < 0.5
                    cross_exp = cross_mask.unsqueeze(1).expand_as(child_data)
                    child_data[:] = torch.where(cross_exp & neuron_mask, parent2_data, child_data)
                elif child_data.dim() == 3:
                    # (k, neurons_out, neurons_in) — swap entire output neurons
                    neuron_mask = (torch.rand(k, child_data.shape[1], device=self.dev) < 0.5).unsqueeze(2)
                    neuron_mask = neuron_mask.expand_as(child_data)
                    cross_exp = cross_mask.unsqueeze(1).unsqueeze(2).expand_as(child_data)
                    child_data[:] = torch.where(cross_exp & neuron_mask, parent2_data, child_data)

            # Crossover children inherit activation ID from one parent randomly
            act_swap = cross_mask & (torch.rand(k, device=self.dev) < 0.5)
            self.act_ids[n_keep:] = torch.where(act_swap, self.act_ids[parent2_idx], self.act_ids[n_keep:])

            # Reproductive traits: inherit from a random parent
            for attr in ["genome_mut_rate", "genome_mut_scale", "genome_explore", "genome_crossover"]:
                child = getattr(self, attr)[n_keep:]
                p2 = getattr(self, attr)[parent2_idx]
                trait_swap = cross_mask & (torch.rand(k, device=self.dev) < 0.5)
                child[:] = torch.where(trait_swap, p2, child)

        # --- V4: PER-GENOME MUTATION using parent's reproductive traits ---
        # Each child is mutated according to the PARENT's mut_rate/mut_scale.
        # This means a parent that evolved high exploration passes that trait
        # to children who are then mutated more aggressively.
        n_keep = len(keep_idx)

        # Get each child's inherited reproductive traits (from parent genome, now in bottom-k)
        child_rates = self.genome_mut_rate[n_keep:]     # (k,) per-genome mutation rate
        child_scales = self.genome_mut_scale[n_keep:]   # (k,) per-genome mutation magnitude
        child_explore = self.genome_explore[n_keep:]    # (k,) per-genome jump probability

        # Encoder + controller weights: mutate using per-genome rate and scale
        for param in [self.enc_w, self.enc_b, self.w1, self.b1, self.w2, self.b2]:
            bottom = param[n_keep:]
            # Expand per-genome rate to match param shape
            rate_exp = child_rates
            scale_exp = child_scales
            for _ in range(len(bottom.shape) - 1):
                rate_exp = rate_exp.unsqueeze(-1)
                scale_exp = scale_exp.unsqueeze(-1)
            mask = torch.rand_like(bottom) < rate_exp
            noise = torch.randn_like(bottom) * scale_exp
            # Explore: some genomes occasionally make large jumps
            explore_exp = child_explore
            for _ in range(len(bottom.shape) - 1):
                explore_exp = explore_exp.unsqueeze(-1)
            jump_mask = torch.rand_like(bottom) < explore_exp
            noise = torch.where(jump_mask, noise * 5.0, noise)  # 5x larger jump mutations
            bottom.add_(noise * mask)

        # Activation params: same per-genome mutation
        for param in [self.act_p1, self.act_p2, self.act_p3, self.act_p4]:
            bottom = param[n_keep:]
            rate_exp = child_rates.unsqueeze(-1)
            scale_exp = child_scales.unsqueeze(-1) * 2.0  # activation params mutate faster
            mask = torch.rand_like(bottom) < rate_exp
            bottom.add_(torch.randn_like(bottom) * scale_exp * mask)
            bottom.clamp_(-5.0, 10.0)

        # Protein params — use FIXED global rate for trust proteins
        # (not per-genome rate). If genomes control their own trust
        # mutation rate they evolve to inflate trust_gain instead of
        # learning to play better. Trend momentum uses per-genome rate.
        fixed_rate = child_mutation_rate  # global fallback rate
        for param, pscale, lo, hi, use_fixed in [
            (self.trend_momentum, 0.2, 0.0, 0.99, False),
            (self.trust_gain, 0.2, 0.1, 10.0, True),
            (self.trust_scale, 0.2, -5.0, 5.0, True),
            (self.trust_decay, 0.2, 0.0, 0.999, True),
        ]:
            bottom = param[n_keep:]
            if use_fixed:
                mask = torch.rand_like(bottom) < fixed_rate
            else:
                rate_exp = child_rates
                for _ in range(len(bottom.shape) - 1):
                    rate_exp = rate_exp.unsqueeze(-1)
                mask = torch.rand_like(bottom) < rate_exp
            noise = torch.randn_like(bottom) * pscale * (bottom.abs() + 1e-9)
            bottom.add_(noise * mask).clamp_(lo, hi)

        # Mutate the reproductive traits themselves (meta-mutation)
        # Children's reproductive strategy can drift from parent's
        meta_rate = 0.1  # 10% chance of changing reproductive strategy
        meta_mask = torch.rand(k, device=self.dev) < meta_rate
        self.genome_mut_rate[n_keep:] += (torch.randn(k, device=self.dev) * 0.01 * meta_mask)
        self.genome_mut_scale[n_keep:] += (torch.randn(k, device=self.dev) * 0.02 * meta_mask)
        self.genome_explore[n_keep:] += (torch.randn(k, device=self.dev) * 0.02 * meta_mask)
        self.genome_crossover[n_keep:] += (torch.randn(k, device=self.dev) * 0.03 * meta_mask)
        self.genome_mut_rate[n_keep:].clamp_(0.01, 0.15)
        self.genome_mut_scale[n_keep:].clamp_(0.01, 0.5)
        self.genome_explore[n_keep:].clamp_(0.0, 0.3)
        self.genome_crossover[n_keep:].clamp_(0.0, 0.8)

        # Activation switch (using per-genome rate)
        switch_mask = torch.rand(k, device=self.dev) < child_rates * 0.1
        if switch_mask.any():
        ...
        self.effective_mutation_rate = self.genome_mut_rate.mean().item()
    ...
    def sync_to_cpu(self, population, tiles=None, scores=None, trust=None):
        ...
        # Sync reproductive traits
        mr = self.genome_mut_rate.cpu().tolist()
        ms = self.genome_mut_scale.cpu().tolist()
        me = self.genome_explore.cpu().tolist()
        mc = self.genome_crossover.cpu().tolist()
        for i, g in enumerate(population.genomes):
            g.repro_mut_rate = mr[i]
            g.repro_mut_scale = ms[i]
            g.repro_explore = me[i]
            g.repro_crossover = mc[i]
